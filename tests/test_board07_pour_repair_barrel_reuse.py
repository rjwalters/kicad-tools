"""Board07's pour repair must bridge to an existing same-net via barrel.

Issue #5507.  ``boards/07-matchgroup-test/generate_design.py``'s
``_repair_pour_connectivity`` ends with a via-hop stage that drops a via
inside *each* copper component and crosses the blockage on another layer.
When the primary-side copper reachable from the stranded component is an
existing via barrel (plus a pad it cannot drill into), that stage could
never place its second via -- a drill inside the barrel violates the
drill-to-drill floor, and a drill outside it is not on the component -- so
the component was printed ``UNREPAIRED`` and its pad stayed open.  That is
the shape of the demonstrated ``+1V2`` open at ``U4.C2``.
...
independent of any retained artifact).  It is red on the pre-fix stage --
``(0, 0)`` with an ``UNREPAIRED`` line -- and green afterwards with exactly
one new drill, the one on the stranded island's side.
"""
...
def generate_design_mod():
    ...
    gp = _load_module("board_07_generate_pcb_bridge", BOARD_DIR / "generate_pcb.py")
    sys.modules["generate_pcb"] = gp
    gs = _load_module("board_07_generate_schematic_bridge", BOARD_DIR / "generate_schematic.py")
    sys.modules["generate_schematic"] = gs
    return _load_module("board_07_generate_design_bridge", BOARD_DIR / "generate_design.py")
