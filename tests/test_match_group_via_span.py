...
@pytest.mark.parametrize(
    ...
    [
        ...
        ((Layer.F_CU, Layer.IN2_CU), Layer.IN1_CU, True, True),
        ((Layer.IN2_CU, Layer.F_CU), Layer.IN1_CU, True, True),
    ],
)
...
@pytest.mark.parametrize(
    "same_net,clearance,via_y", [(True, 0.2, 5), (False, None, 5), (False, 0.2, 6)]
)
def test_via_span_preserves_same_net_disabled_and_clear_controls(same_net, clearance, via_y):
    net = 1 if same_net else 2
    via = Via(5, via_y, 0.3, 0.6, (Layer.F_CU, Layer.B_CU), net=net)
    assert (
        _post_insertion_clearance_detail_group(
            new_segments=[Segment(4, 5, 6, 5, 0.2, Layer.IN1_CU, net=1)],
            candidate_net_id=1,
            group_net_ids={1, 2},
            routes_by_net={net: Route(net=net, net_name="control", vias=[via])},
            intra_group_clearance_mm=0.2,
            via_clearance_mm=clearance,
        )
        is None
    )
