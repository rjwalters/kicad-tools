...
def test_native_staged_pad_drill_repair_preserves_connectivity(
    ...
    # Use the reviewed successor's effective factory rules rather than the
    # archived DRU that silently fails native rule enforcement.
    dru = candidate.with_suffix(".kicad_dru")
    dru_bytes = (OUTPUT_DIR.parent / "regression-input" / dru.name).read_bytes()
    dru.write_bytes(dru_bytes)
    rules = get_profile("jlcpcb").get_design_rules(layers=4)
    assert len(ViaInPadRule().check(pcb, rules).violations) == 7
    ...
    assert dru.read_bytes() == dru_bytes
