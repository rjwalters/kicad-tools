...
class TestRouteExitCodeDocumentation:
    ...
    Issues #4559 / #5348: ``route_cmd.main`` became a thin process-state-guard
    wrapper around ``route_cmd._run_main_impl``; the exit-code logic these
    ...
    def test_source_documents_exit_code_2_for_partial(self):
        ...
        source = inspect.getsource(route_cmd._run_main_impl)
        assert "return 2" in source, (
            "route_cmd._run_main_impl must contain 'return 2' for partial routing"
        )
        assert "Partial routing" in source, (
            "route_cmd._run_main_impl must document partial routing for exit code 2"
        )
    ...
    def test_source_documents_exit_code_3(self):
        ...
        assert "return 3" in source, (
            "route_cmd._run_main_impl must contain 'return 3' for DRC-only failures"
        )
        assert "return 0" in source, "route_cmd._run_main_impl must contain 'return 0' for success"
        assert "return 1" in source, (
            "route_cmd._run_main_impl must contain 'return 1' for fatal failure"
        )
    ...
    def test_source_documents_threshold_semantics(self):
        ...
        assert "min_completion" in source, (
            "route_cmd._run_main_impl must reference min_completion in exit code logic"
        )
        assert "meets_threshold" in source, (
            "route_cmd._run_main_impl must use meets_threshold variable"
        )
