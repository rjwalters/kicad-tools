...
def routed_drc_report() -> tuple:
    """Route the diagnostic PCB once and run DRC, shared across tests.

    Returns (report, stats) tuple.
    """
    pcb_path = Path(__file__).parent / "fixtures" / "routing-diagnostic.kicad_pcb"
    rules = DesignRules(
        trace_width=0.2,
        trace_clearance=0.2,
        via_drill=0.3,
    ...
    write_routed_pcb(router, pcb_path, output_path)
...
class TestRoutingDRCCompliance:
    ...
    def test_routing_produces_no_drc_violations(
        ...
        report, stats = routed_drc_report
...
class TestPadClearancePreservation:
    ...
    def test_no_clearance_violations_after_multiple_ripups(
        ...
        Uses the shared routed fixture (5 iterations of rip-up/reroute).
        If the bug from #292 regresses, routes may pass too close to pads.
        """
        report, stats = routed_drc_report
...
class TestIterationStability:
    ...
    def test_more_iterations_not_worse(
        ...
        for iters, count in zip([3, 5], violation_counts, strict=True):
