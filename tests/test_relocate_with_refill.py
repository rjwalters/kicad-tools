...
def test_native_refill_requests_all_track_errors(tmp_path, monkeypatch):
    import subprocess

    board = tmp_path / "probe.kicad_pcb"

    def native(argv, **kwargs):
        assert "--all-track-errors" in argv
        assert "--refill-zones" in argv and "--save-board" in argv
        assert kwargs["timeout"] == 180
        Path(argv[argv.index("--output") + 1]).write_text(json.dumps(_empty()))
