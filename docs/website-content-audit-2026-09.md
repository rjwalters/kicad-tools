...
`output/readiness.json` (tracked for boards 00–07 and 09, and validated at
build time by `readiness.ts`'s `loadReadiness`). A fresh checkout contains
these readiness reports even though it lacks generated `board.json` files.
At the reviewed tree, all Board09 readiness input hashes match the committed
files; its evidence is available, but `loadBoard` returns a stub before
loading it when `board.json` is absent. Two concrete, verified findings:
