...
docs/quickstart page, no "about" page, and no link anywhere on the site back
to the repository's `README.md` or `docs/` tree specifically — "View on
GitHub" goes to the repo root, which is the only escape hatch to more detail.
...
`output/readiness.json` (also never committed, validated at build time by
`readiness.ts`'s `loadReadiness`). Two concrete, verified findings:
...
**F11 — no path to documentation from the site itself.** `Header.astro`'s
nav is exactly `kicad-tools` (home) / `Benchmarks` / `View on GitHub` (repo
root). There is no link to `README.md`, a quickstart, or any `docs/` entry
point. A visitor who wants more than the gallery has to land on the raw
GitHub repository tree unaided. See **Recommendation 2G**.
