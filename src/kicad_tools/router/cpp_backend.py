...
class CppGrid:
    ...
    def install_fixed_fills(self, fills) -> None:
        ...
        for layer, clearance, rings, via_clearance in fills.native_polygons(
            include_via_clearance=True
        ):
            self._impl.add_fixed_fill(
                layer, clearance, rings, -1.0 if via_clearance is None else via_clearance
            )
