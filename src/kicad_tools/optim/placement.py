...
This module provides a force-directed placement algorithm using:
...
class PlacementOptimizer:
    ...
    def compute_forces_and_torques(self) -> tuple[dict[str, Vector2D], dict[str, float]]:
        ...
        # 4. Rotation potential torque (torsion spring toward 90 degree slots)
        for comp in self.components:
            if not comp.fixed:
    ...
    def compute_energy(self) -> float:
        ...
        - Rotation potential energy (torsion spring toward 90 degree slots)
        """
        kinetic = 0.0
        for comp in self.components:
            if not comp.fixed:
        ...
        # Rotation potential energy (torsion springs toward 90 degree slots)
        for comp in self.components:
            if not comp.fixed:
    ...
    def snap_rotations_to_90(self):
        """
        Force all components to exact 90 degree orientations.

        Call after optimization to ensure components are at 0, 90, 180, or 270 degrees.
        """
