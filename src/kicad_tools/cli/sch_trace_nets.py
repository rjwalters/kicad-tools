#!/usr/bin/env python3
"""
Trace and analyze nets in a KiCad schematic.

Usage:
    python3 sch-trace-nets.py <schematic.kicad_sch> [options]

Options:
    --format {table,json}      Output format (default: table)
    --net <name>               Trace a specific net by label
    --stats                    Show net statistics only

Examples:
    # List all nets
    python3 sch-trace-nets.py clock.kicad_sch

    # Trace a specific net
    python3 sch-trace-nets.py clock.kicad_sch --net MCLK_DAC

    # Get net statistics
    python3 sch-trace-nets.py clock.kicad_sch --stats
"""
...
def output_all_table(nets):
    ...
    for net in nets:
        total_length = sum(w.length for w in net.wires)
        label_mark = "✓" if net.has_label else ""
        conn_types = []
        for conn in net.connections:
            if conn.type == "pin":
                conn_types.append(f"{conn.reference}.{conn.pin_number}")
            elif conn.type == "label":
                pass  # Already shown in name
            elif conn.type == "junction":
