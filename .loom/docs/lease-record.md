...
## Publish-side yield-exclusion, closed (Issue #5331)
...
#5331 (a 36-hour, 4+-host claim/yield thrash on issue #5240, 137 of 160
...
Rust code in the separate `loom-daemon` binary's source repository — not
vendored into this repo, and out of scope for this fix. The #5240 incident's
...
contention; #5331's fix closes this repo's own contributing gap on the
in-session write path, but does not by itself prove the daemon-dispatched
path converges — that requires a companion investigation upstream (tracked
separately) and live-fleet confirmation post-merge, since a race of this
