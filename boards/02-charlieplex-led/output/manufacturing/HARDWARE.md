...
All schematic and PCB generators call `hardware_design.py`; pin assignments and LED topology come from `design_spec.py`. Physical pad geometry comes from the installed KiCad standard libraries. The routed output increases silkscreen strokes to 0.15 mm and applies the two reviewed escape-via corrections in `finalize_routing.py`. Generate into a separate directory and validate ERC, label/copper LVS and native DRC before replacing published outputs.
