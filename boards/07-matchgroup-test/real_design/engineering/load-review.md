...
- **DQ bidirectional evaluation**: `external_load()` now evaluates both directions explicitly -- SDRAM-drives/MCU-receives (10 pF engineering reserve) and MCU-drives/SDRAM-receives (6.5 pF ISSI maximum) -- and reports both in `external_load_estimates[net]["directions"]`, rather than folding them into one opaque number. Each direction reports its driver limit, limit provenance, margin and pass/fail result. The production checker rejects if either direction fails. Compatibility scalar fields follow the smallest margin, so a lower-load direction can govern when its limit is smaller. The MCU-driven 30 pF budget is the ST characterization load; the SDRAM-driven 30 pF budget is explicitly an engineering screen with its driver limit still unconfirmed. Regression controls exercise asymmetric synthetic 30/20 pF budgets, including through the production checker; they are not vendor specifications.
