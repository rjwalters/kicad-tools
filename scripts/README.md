...
| `install-kct.sh` | Install kicad-tools into a consumer PCB-design repo: uv dependency + vendored `ci/` gate scripts + `.kct/CONVENTIONS.md`, plus per-client workflow content selected via `--client claude\|codex\|both` (default `claude`, backward-compatible) — Claude gets `.claude/commands/kct/` skills + a guarded `CLAUDE.md` block, Codex gets `.agents/skills/kct-<name>/SKILL.md` + a guarded `AGENTS.md` block, both generated from the same source `.md` files (#4905). |
